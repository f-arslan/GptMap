...
fun Type.toKTypeProjection(): KTypeProjection = when (this) {
    is Class<*> -> this.kotlin.toInvariantFlexibleProjection(
        if (this.isArray) listOf(this.componentType.toKTypeProjection()) else emptyList()
    )
        ...
        erasure.toInvariantFlexibleProjection(
            (
                    erasure.typeParameters.zip(actualTypeArguments).map { (parameter, argument) ->
                        ...
                        projection.takeIf {
                            // Get rid of use-site projections on arguments, where the corresponding parameters
                            // already have a declaration-site projection
    ...
    is WildcardType -> when {
        ...
        // This looks impossible to obtain through Java reflection API,
        // but someone may construct and pass such an instance here anyway
...
class RandomInstanceProducer(
    ...
    private fun makeRandomString(random: Random) =
        ...
        )).asSequence().map { makeRandomChar(random) }
