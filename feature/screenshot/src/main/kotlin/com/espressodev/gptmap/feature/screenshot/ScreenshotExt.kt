...
sealed class ScreenshotError(message: String) : Exception(message) {
    ...
    class TimeoutError : ScreenshotError("A timeout occurred while trying to acquire a buffer from the source.")
...
suspend fun View.screenshot(bounds: Rect): ImageResult = suspendCancellableCoroutine { continuation ->
    try {
        ...
        PixelCopy.request(
            (this.context as Activity).window, // if it's a fragment, it can throw an error. We are using it in compose, so it's an activity
            ...
            { result ->
                when (result) {
                    ...
                    PixelCopy.ERROR_DESTINATION_INVALID -> continuation.resumeWithException(ScreenshotError.DestinationInvalidError())
                    PixelCopy.ERROR_SOURCE_INVALID -> continuation.resumeWithException(ScreenshotError.SourceInvalidError())
                    PixelCopy.ERROR_TIMEOUT -> continuation.resumeWithException(ScreenshotError.TimeoutError())
                    PixelCopy.ERROR_SOURCE_NO_DATA -> continuation.resumeWithException(ScreenshotError.SourceNoDataError())
